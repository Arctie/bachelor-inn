...
func stop_loop(loop_type: SoundEffect.SOUND_EFFECT_TYPE) -> void:
	...
	var loop_data: Dictionary = active_looping_sounds[loop_type]
	var loop_player: Node = loop_data.player
...
func change_loop_to_clip(loop_type: SoundEffect.SOUND_EFFECT_TYPE, clip_index: int, is_final_clip: bool = false) -> void:
	if active_looping_sounds.has(loop_type):
		...
		if loop_player != null and is_instance_valid(loop_player):
			var playback: AudioStreamPlayback = loop_player.get_stream_playback()
			if playback != null:
				playback.switch_to_clip(clip_index)
				
				if is_final_clip:
					var stream: AudioStreamInteractive = loop_player.stream as AudioStreamInteractive
					if stream != null:
						var clip_stream: AudioStream = stream.get_clip_stream(clip_index)
						if clip_stream != null:
							var clip_length: float = clip_stream.get_length()
							if clip_length > 0.05:
								var cleanup_timer: SceneTreeTimer = get_tree().create_timer(clip_length)
								loop_data["final_clip_timer"] = cleanup_timer
								
								var captured_loop_type: SoundEffect.SOUND_EFFECT_TYPE = loop_type
								var captured_loop_player: Node = loop_player
								
								cleanup_timer.timeout.connect(func() -> void:
									if active_looping_sounds.has(captured_loop_type) and is_instance_valid(captured_loop_player):
										var sound_effect: SoundEffect = sound_effect_dict.get(captured_loop_type)
										if sound_effect:
											sound_effect.on_audio_finished()
...
func change_loop_layer_volume(type: SoundEffect.SOUND_EFFECT_TYPE, layer: int, end_db: float, fade_duration: float = 0.0) -> void:
	if active_looping_sounds.has(type):
		var loop_data: Dictionary = active_looping_sounds[type]
		var loop_player: Node = loop_data.player
		var stream: AudioStreamSynchronized = loop_player.stream as AudioStreamSynchronized
		if sound_effect_dict.has(type):
		...
		if fade_duration > 0.0:
			...
			var current_volume: float = stream.get_sync_stream_volume(layer)
...
func stop_music(type: MusicTrack.TRACK_TYPE, fade_time: float = 0.0) -> void:
	if active_music_dict.has(type):
		var music: AudioStreamPlayer = active_music_dict[type]
		if music_tracks_dict.has(type):
...
func stop_all_music(fade_time: float = 0.0) -> void:
	...
	for track_type: MusicTrack.TRACK_TYPE in active_music_dict.keys():
	...
	for track_type: MusicTrack.TRACK_TYPE in active_music_dict.keys():
		var music: AudioStreamPlayer = active_music_dict[track_type]
		if fade_time > 0.0:
			var fade_tween := create_tween().set_ease(Tween.EASE_IN_OUT)
			fade_tween.tween_property(music, "volume_db", -80.0, fade_time)
			fade_tween.finished.connect(func() -> void:
...
func change_music_layer_volume(type: MusicTrack.TRACK_TYPE, layer: int, end_db: float, fade_duration: float = 0.0) -> void:
	if active_music_dict.has(type):
		var music: AudioStreamPlayer = active_music_dict[type]
		var stream: AudioStreamSynchronized = music.stream as AudioStreamSynchronized
		if music_tracks_dict.has(type):
...
func get_managed_player(player_name: String) -> Node:
	var player: Node = managed_players.get(player_name)
...
func stop_all_managed_players() -> void:
	for player: Node in managed_players.values():
...
func stop_all_audio(music_fade_time: float = 0.0) -> void:
	...
	for loop_type: SoundEffect.SOUND_EFFECT_TYPE in active_looping_sounds.keys():
		var loop_player: Node = active_looping_sounds[loop_type].player
		if is_instance_valid(loop_player):
			loop_player.stop()
			loop_player.queue_free()
...
func _validate_all_audiotypes_content() -> void:
	...
	var unassigned_soundeffect_types: Array = []
	for i in range(sound_effects.size()):
		var sound_effect: SoundEffect = sound_effects[i]
		var effect_type: SoundEffect.SOUND_EFFECT_TYPE = sound_effect.type
	...
	if warn_on_missing_sound_assignments:
		var missing_sound_effect_values: Array = []
		var used_sound_types: Array = sound_effect_dict.keys()
		for effect_type: SoundEffect.SOUND_EFFECT_TYPE in SoundEffect.SOUND_EFFECT_TYPE.values():
	...
	var unassigned_tracks: Array = []
	for i in range(music_tracks.size()):
		var music_track: MusicTrack = music_tracks[i]
		var track_type: MusicTrack.TRACK_TYPE = music_track.track_type
	...
	if warn_on_missing_sound_assignments:
		var missing_music_track_values: Array = []
		var used_track_types: Array = music_tracks_dict.keys()
		for track_type: MusicTrack.TRACK_TYPE in MusicTrack.TRACK_TYPE.values():
...
func _get_valid_bus_name(requested: StringName) -> StringName:
	var requested_text: String = String(requested).strip_edges()
...
func _start_loop_2d(loop_type: SoundEffect.SOUND_EFFECT_TYPE, parent_node: Node2D) -> void:
	if sound_effect_dict.has(loop_type):
		...
		if loop_sound.has_open_limit():
			...
			loop_audio.finished.connect(func() -> void:
