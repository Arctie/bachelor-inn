...
func _register_player() -> void:
	var manager_node: Node = _get_manager_node()
	if not manager_node:
...
func _get_manager_node() -> Node:
	var resolved_type: int = manager_type
	if manager_type == 0:
	...
	var manager_name: String = manager_name_2d if resolved_type == 1 else manager_name_3d
